...
// mockWeather 的内部实现，技能定义与签名保持不变。
func GetWeather() skill.Skill {
	return skill.NewFunc(skill.Manifest{
		Definition: skill.Definition{
			Name:        "get_weather",
			Description: "查询指定城市的天气（当前为模拟数据）。",
			InputSchema: skill.Schema("获取天气的入参", map[string]any{
				"city": skill.StringProperty("城市名称"),
				"unit": skill.StringProperty("温度单位，celsius 或 fahrenheit，默认 celsius"),
			}, "city"),
		},
		Version:      "1.0.0",
		Instructions: "根据 city 查询该城市的模拟天气；unit 可选（celsius / fahrenheit），默认 celsius。",
